...
def run():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("--setup", "setup"):
        ...
        if status["authenticated"]:
            ...
            storage = status.get("storage", "file")
            print(f"   Token storage: {storage}")
        else:
...
def _run_doctor():
    ...
    status = get_auth_status()
    if status["authenticated"]:
        print(f"Logged in as {status['email']} (storage: {status.get('storage', 'file')})")
    else:
        print("Not logged in")
    ...
    print("=" * 50)
    if not issues:
        print("All checks passed. Restart Claude Code and use /boost.")
    else:
...
def _run_delete_data():
    ...
    print("This will permanently delete ALL your ClaudeBoost data.")
    ...
    for key, done in deleted.items():
        status = "done" if done else "not found or skipped"
        print(f"   {key}: {status}")
    print()
