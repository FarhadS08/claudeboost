...
const steps = [
  {
    ...
    visual: (
      <div className="font-mono text-sm bg-muted/50 rounded-xl p-4 border border-border">
        <span className="text-primary">&#10095;</span>{" "}
        <span className="text-foreground/70">build me an API endpoint for user auth</span>
      </div>
    ),
  },
  {
        ...
        {["Specificity", "Context", "Constraints", "Tests", "Structure", "Output"].map(
          (dim, i) => (
            <div key={dim} className="flex-1">
              <div className="h-16 bg-muted/50 rounded-lg border border-border relative overflow-hidden">
                <div
                  className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-primary/30 to-primary/5 rounded-b-lg"
                  style={{ height: `${50 + i * 8}%` }}
                />
              </div>
    ...
    visual: (
      <div className="flex gap-3">
        ...
        <div className="flex-1 bg-primary/5 rounded-xl p-3 border border-primary/20">
          ...
          <p className="text-xs font-semibold text-primary mt-2">27/30</p>
        </div>
      </div>
    ),
  },
      ...
      <div className="flex items-center gap-4 bg-muted/50 rounded-xl p-4 border border-border">
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((star) => (
            <span
              key={star}
              className={`text-lg ${star <= 4 ? "text-primary" : "text-muted-foreground/30"}`}
            >
              &#9733;
            </span>
          ))}
      ...
      <div className="max-w-5xl mx-auto relative">
        <div className="text-center mb-20">
          ...
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight">
            Four steps to{" "}
            <span className="text-primary">10x better prompts</span>
          </h2>
        </div>
          ...
          {steps.map((step, i) => (
            ...
            >
              <div className="flex-1 space-y-4">
                <span className="text-6xl font-bold text-muted/50 font-mono">
                  {step.number}
                </span>
