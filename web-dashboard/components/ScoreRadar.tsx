  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {DIMS.map((dim, i) => {
          ...
          return (
            <g key={i}>
              ...
              {/* Invisible fat hitzone for hover */}
              {interactive && (
                <line
        ...
        {showLabels && DIMS.map((dim, i) => {
          ...
          return (
            <text
              ...
              fontWeight={isHovered ? "800" : improved ? "600" : "400"}
              fontFamily="Inter, sans-serif"
              style={{
                opacity: animated ? 1 : 0,
                transition: `opacity 0.4s ease ${0.3 + i * 0.05}s, font-size 0.2s ease, fill 0.2s ease`,
                cursor: interactive ? "pointer" : undefined,
              }}
              onMouseEnter={() => interactive && setHoveredDim(dim)}
              onMouseLeave={() => interactive && setHoveredDim(null)}
            >
              {DIM_SHORT[dim]}
            </text>
          );
        ...
        {/* Before shape */}
        <path
          d={animated ? beforePath : zeroPath}
        ...
        {showLabels && DIMS.map((dim, i) => {
          ...
          return (
            <circle
              ...
              style={{
                transition: `cx 0.15s ease, cy 0.15s ease, r 0.2s ease, opacity 0.4s ease ${0.3 + i * 0.06}s`,
                cursor: interactive ? "pointer" : undefined,
              }}
              onMouseEnter={() => interactive && setHoveredDim(dim)}
              onMouseLeave={() => interactive && setHoveredDim(null)}
      ...
      {interactive && hoveredDim && (
        <div
          className="absolute top-0 right-0 bg-[rgba(0,0,0,0.85)] border border-[rgba(255,255,255,0.1)] rounded-xl px-4 py-3 pointer-events-none z-10 animate-fade-in"
          style={{ minWidth: 140 }}
        >
